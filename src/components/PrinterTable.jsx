...
export default function PrinterTable({
  ...
  const handleReboot = async (printerId, ip, modelo, tipoReinicio = "warm") => {
    ...
    try {
      ...
      const estadoRes = await fetch(
        `http://192.168.8.165:3001/api/impresoras/${printerId}/status`
      );
      ...
      const res = await fetch(
        `http://192.168.8.165:3001/api/impresoras/${printerId}/reboot`,
        {
          method: 'POST',
          headers: {
  ...
  const checkAllPrintersStatus = async () => {
    ...
    try {
      const res = await fetch(
        "http://192.168.8.165:3001/api/impresoras/status"
      );
  ...
  `;
    ...
    try {
      const res = await fetch(
        `http://192.168.8.165:3001/api/impresoras/${impresoraId}/print`,
        {
          method: "POST",
          body: formData,
        }
